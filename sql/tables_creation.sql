...
CREATE TABLE jugadors (ID INTEGER NOT NULL UNIQUE AUTO_INCREMENT, usr_id INTEGER, jug_nom varchar(50),jug_punts INTEGER, id_partida INTEGER NOT NULL, jug_aciertos INTEGER, PRIMARY KEY(ID), FOREIGN KEY (usr_id) REFERENCES usuaris_registrats(ID), FOREIGN KEY(id_partida) REFERENCES partida(id));
